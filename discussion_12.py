...
def create_employee_table(cur, conn):
    cur.execute('CREATE TABLE IF NOT EXISTS employees (employees_id INTEGER PRIMARY KEY , first_name TEXT, last_name TEXT, job_id INTEGER, hire_date TEXT, salary INTEGER')
    conn.commit()
...
def add_employee(filename, cur, conn):
    ...
    for i in json_data:
        cur.execute('INSERT OR IGNORE INTO employees (employee_id, first_name, last_name, job_id, hire_dates, salary) VALUES (?, ?, ?, ?, ?,?)',  i['employee_id'], i['first_name'], i['last_name'], i['hire_date'], i['job_id'], i['salary'])
    conn.commit()
